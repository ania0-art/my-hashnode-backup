...
* 以及任何会导致“你以为写了 entry，其实只是在改某些无关位”的情况 咬住指令模拟必须具备处理复杂原子操作的能力，如果解码器不支持某些偏门指令导致计算出的 NewValue 错误，会导致页表状态与 EPT 状态去同步，引发系统崩溃。（在测试windows时候就出现过严重问题）
